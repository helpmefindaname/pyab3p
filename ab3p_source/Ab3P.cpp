...
std::vector<AbbrOut> Ab3P::get_abbrs(char *text) {
  vector<AbbrOut> abbrs;

  if (!text[0])
    return abbrs; // skip empty line
...
void Ab3P::try_strats(const char *psf, const char *plf, bool swap,
  ...
  if (!util.group_sf(psf, plf, sfg))
    return;
  if (swap)
    if (!util.exist_upperal(psf))
      return;
  ...
  for (int j = 0; j < strats.size(); j++) {
    AbbrStra *strat = util.strat_factory(strats[j]); // set a paticular strategy
    ...
    if (strat->strategy(sf, plf)) {                  // case sensitive
      ...
      if (strat->lf_ok(psf, strat->lf)) {
        ...
        map<string, double>::iterator p = stratPrec.find(sfg + strats[j]);
        if (p == stratPrec.end()) {
          throw std::runtime_error("No precision assigned");
        }
...
void Ab3P::try_strats_pot_abbr(const Pot_Abbr &abb, bool swap,
  ...
  if (!util.group_sf(abb.abbs, abb.abbl, sfg))
    return;
  if (swap)
    if (!util.exist_upperal(abb.abbs))
      return;
    ...
    if (strat->strategy(sf, abb.abbl)) {             // case sensitive
      ...
      if (strat->lf_ok(abb.abbs, strat->lf)) {
        ...
        if (abb.abbl_tokens.size() != strat->ntk) {
          cout << "different number of tokens: " << abb.abbl_tokens.size()
               << ' ' << strat->ntk << '\n';
